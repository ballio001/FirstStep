...
public class TriggeredSpawnerV2Inspector : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        var newOutside = EditorGUILayout.Toggle(new GUIContent("Spawn Outside Pool", "If this is checked, everything spawned from this will not reside under Pool Boss in the Hierarchy, but instead with no parent Game Object. Not that this can change the size of what's spawned depending on what you have chosen for Spawn Scale Mode."), _settings.spawnOutsidePool);
    ...
    private void RenderTriggeredWave(ref List<TriggeredWaveSpecifics> waveList, TriggeredWaveSpecifics waveSetting, string toggleText, TriggeredSpawner.EventType eventType, int? itemIndex = null)
    {
        ...
        if (eventType == TriggeredSpawner.EventType.CustomEvent)
        {
            ...
            DTInspectorUtility.AddHelpIconNoStyle("http://www.dtdevtools.com/docs/coregamekit/TriggeredSpawners.htm#WaveSettings");
        ...
        var newDType = (LevelSettings.DistanceType)EditorGUILayout.EnumPopup(new GUIContent("Distance Type", "This is used on all distance settings (Position, Incremental, Randomization). Use 'World' unless you need to account for rotation of the spawner."), waveSetting.randomDistanceType);
        if (newDType != waveSetting.randomDistanceType) {
            UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "change Distance Type");
            waveSetting.randomDistanceType = newDType;
            _changedWave = waveSetting;
        }
        ...
        DTInspectorUtility.StartGroupHeader(1);
        var newScale = (WaveSpecifics.ScaleMode)EditorGUILayout.EnumPopup("Spawn Scale Mode", waveSetting.curScaleMode);
        if (newScale != waveSetting.curScaleMode) {
            UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "change Scale Mode");
            waveSetting.curScaleMode = newScale;
            _changedWave = waveSetting;
        }
        EditorGUILayout.EndVertical();

        switch (waveSetting.curScaleMode) {
            case WaveSpecifics.ScaleMode.CustomScale:
                var newCustScale = EditorGUILayout.Vector3Field("Custom Scale", waveSetting.customScale);
                if (newCustScale != waveSetting.customScale) {
                    UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "change Custom Scale");
                    waveSetting.customScale = newCustScale;
                    _changedWave = waveSetting;
                }
                break;
            case WaveSpecifics.ScaleMode.PercentOfSpawnerScale:
                var newPercent = EditorGUILayout.FloatField("Percent of Spawner Scale", waveSetting.percentOfSpawnerScale);
                if (newPercent != waveSetting.percentOfSpawnerScale) {
                    UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "change Percent of Spawner Scale");
                    waveSetting.percentOfSpawnerScale = newPercent;
                    _changedWave = waveSetting;
                }
                break;
        }
        ...
        if (TriggeredSpawner.eventsThatCanRepeatWave.Contains(eventType))
        {
            ...
            if (waveSetting.enableRepeatWave)
            {
                ...
                KillerVariablesHelper.DisplayKillerFloat(ref _isDirty, waveSetting.repeatWavePauseSec, "Repeat Pause Min", _settings);
                KillerVariablesHelper.DisplayKillerFloat(ref _isDirty, waveSetting.repeatWavePauseMax, "Repeat Pause Max", _settings);
