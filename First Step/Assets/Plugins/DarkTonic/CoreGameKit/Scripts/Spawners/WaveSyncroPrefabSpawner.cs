    ...
    public class WaveSyncroPrefabSpawner : MonoBehaviour {
        ...
        public void SpawnWaveVisual(WaveSpecifics wave) {
            ...
            for (var i = 0; i < nbrToSpawn; i++) {
                var spawnPosition = GetSpawnPositionForVisualization(wave, transform, i);
                ...
                AffectScaleforVisualization(wave, spawned.transform);
        ...
        protected virtual void AffectScale(Transform spawnedTrans) {
            var originalParent = spawnedTrans.parent;

            switch (_currentWave.curScaleMode) {
                case WaveSpecifics.ScaleMode.UsePrefabScale:
                    // do nothing
                    break;
                case WaveSpecifics.ScaleMode.UseSpawnerScale:
                    spawnedTrans.parent = null;
                    spawnedTrans.localScale = transform.GetGlobalScale();
                    spawnedTrans.parent = originalParent;
                    break;
                case WaveSpecifics.ScaleMode.PercentOfSpawnerScale:
                    spawnedTrans.parent = null;
                    spawnedTrans.localScale = transform.GetGlobalScale() * (_currentWave.percentOfSpawnerScale / 100);
                    spawnedTrans.parent = originalParent;
                    break;
                case WaveSpecifics.ScaleMode.CustomScale:
                    spawnedTrans.parent = null;
                    spawnedTrans.localScale = _currentWave.customScale;
                    spawnedTrans.parent = originalParent;
                    break;
            }
        }
        ...
        protected virtual Vector3 GetSpawnPosition(Vector3 pos, int itemSpawnedIndex) {
            ...
            if (!_currentWave.enableIncrements || itemSpawnedIndex <= 0) {
                switch (_currentWave.randomDistanceType) {
                    case LevelSettings.DistanceType.Local:
                        var worldOffset = transform.rotation * addVector;
                        return pos + worldOffset;
                    case LevelSettings.DistanceType.World:
                    default:
        ...
        private void SpawnBonusPrefabIfSelected(Transform transOfLastKilled) { // only gets called by non-final waves
            if (transOfLastKilled == null) {
                return;
            }
            if (_currentWave.bonusRepeatToUseItem == LevelSettings.RepeatToUseItem.All) {
                SpawnBonusPrefabIfAny(transOfLastKilled);
            }
        }
        ...
        private void SpawnBonusPrefabIfAny(Transform transOfLastKilled) {
            ...
            Vector3 spawnPosition = transOfLastKilled.position;
        ...
        private void MaybeRepeatWave() {
            ...
            if (LevelSettings.PreviousWaveInfo.waveType == LevelSettings.WaveType.Elimination) {
                switch (_currentWave.curWaveRepeatMode) {
                    case WaveSpecifics.RepeatWaveMode.NumberOfRepetitions:
                    case WaveSpecifics.RepeatWaveMode.Endless:
                        SpawnBonusPrefabIfSelected(_lastPrefabKilled);
                        WaveRepeat();
                        break;
                    case WaveSpecifics.RepeatWaveMode.UntilWorldVariableAbove:
                        // ReSharper disable once ForCanBeConvertedToForeach
                        ...
                        if (!allPassed) {
                            ...
                            SpawnBonusPrefabIfAny(_lastPrefabKilled);
        ...
        private void CheckForWaveRepeat() {
            ...
            if (_currentWave.repeatWaveUntilNew && LevelSettings.PreviousWaveInfo.waveType == LevelSettings.WaveType.Timed
                && (_currentWave.curTimedRepeatWaveMode == WaveSpecifics.TimedRepeatWaveMode.StrictTimeStyle || hasNoneLeft)) {
        ...
        private static Vector3 GetSpawnPositionForVisualization(WaveSpecifics wave, Transform spawnerTrans, int itemSpawnedIndex) {
            ...
            if (!wave.enableIncrements || itemSpawnedIndex <= 0) {
                switch (wave.randomDistanceType) {
                    case LevelSettings.DistanceType.Local:
                        var worldOffset = spawnerTrans.rotation * addVector;
                        return pos + worldOffset;
                    case LevelSettings.DistanceType.World:
                    default:
                        return pos + addVector;
        ...
        private void AffectScaleforVisualization(WaveSpecifics wave, Transform spawnedTrans) {
            ...
            switch (wave.curScaleMode) {
                ...
                case WaveSpecifics.ScaleMode.PercentOfSpawnerScale:
                    spawnedTrans.parent = null;
                    spawnedTrans.localScale = transform.GetGlobalScale() * (wave.percentOfSpawnerScale / 100);
                    spawnedTrans.parent = originalParent;
                    break;
                case WaveSpecifics.ScaleMode.CustomScale:
                    spawnedTrans.parent = null;
                    spawnedTrans.localScale = wave.customScale;
                    spawnedTrans.parent = originalParent;
                    break;
            }
        }
