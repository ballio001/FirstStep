    ...
    public class TriggeredSpawnerV2 : MonoBehaviour, ICgkEventReceiver {
        ...
        protected virtual Vector3 GetSpawnPosition(Vector3 pos, int itemSpawnedIndex, TriggeredWaveMetaData wave) {
            ...
            if (!currentWave.enableIncrements || itemSpawnedIndex <= 0) {
                switch (currentWave.randomDistanceType) {
        ...
        protected virtual void AffectScale(Transform spawnedTrans, TriggeredWaveMetaData wave) {
            ...
            switch (wave.waveSpec.curScaleMode) {
                ...
                case WaveSpecifics.ScaleMode.PercentOfSpawnerScale:
                    spawnedTrans.parent = null;
                    spawnedTrans.localScale = transform.GetGlobalScale() * (wave.waveSpec.percentOfSpawnerScale / 100);
                    spawnedTrans.parent = originalParent;
                    break;
                case WaveSpecifics.ScaleMode.CustomScale:
                    spawnedTrans.parent = null;
                    spawnedTrans.localScale = wave.waveSpec.customScale;
                    spawnedTrans.parent = originalParent;
                    break;
            }
        }
        ...
        private void SpawnFromWaveMeta(TriggeredWaveMetaData wave, TriggeredSpawner.EventType eType) {
            ...
            if (wave.waveFinishedSpawning // done spawning, wait
                ...
                if (!wave.waveFinishedSpawning
                    ...
                    || !(Time.time - WaveEndTime(wave) > wave.wavePauseTime)) {
            ...
            for (var i = 0; i < numberToSpawn; i++) {
                if (CanSpawnOne()) {
                    ...
                    AffectScale(spawnedPrefab, wave);
        ...
        private bool SetupNextWave(TriggeredWaveSpecifics newWave, TriggeredSpawner.EventType eventType, int repetitionNumber = 0, int repetitionNumberWithReset = 0) {
            ...
            var randomPause = Random.Range(newWave.repeatWavePauseSec.Value, newWave.repeatWavePauseMax.Value);
        ...
        private void CheckForValidVariablesForWave(TriggeredWaveSpecifics wave, TriggeredSpawner.EventType eType) {
            ...
            wave.repeatWavePauseSec.LogIfInvalid(Trans, "Repeat Pause Min", null, null, eType.ToString());
            wave.repeatWavePauseMax.LogIfInvalid(Trans, "Repeat Pause Max", null, null, eType.ToString());
