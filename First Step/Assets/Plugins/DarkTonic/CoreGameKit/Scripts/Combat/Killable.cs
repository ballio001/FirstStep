...
namespace DarkTonic.CoreGameKit {
	...
	public class Killable : MonoBehaviour {
		...
		private void DisableColliders() {
			if (Colidrs != null) {
				for (var i = 0; i < Colidrs.Count; i++) {
					Colidrs[i].enabled = false;
				}
			}
			
			if (Colidrs2D != null) {
				for (var i = 0; i < Colidrs2D.Count; i++) {
					Colidrs2D[i].enabled = false;
				}
			}
		}
		...
		private void EnableColliders() {
			if (Colidrs != null) {
				for (var i = 0; i < Colidrs.Count; i++) {
					Colidrs[i].enabled = true;
				}
			}
			
			if (Colidrs2D != null) {
				for (var i = 0; i < Colidrs2D.Count; i++) {
					Colidrs2D[i].enabled = true;
				}
			}
		}
		...
		public List<Collider> Colidrs {
			get {
				...
				if (_colliders == null) {
					...
					_colliders = new List<Collider>(allColliders.Length);
					_colliders.AddRange(allColliders);
				}
		...
		private List<Collider2D> Colidrs2D {
			get {
				...
				if (_colliders2D == null) {
					var all2DColliders = GetComponents<Collider2D>();

					_colliders2D = new List<Collider2D>(all2DColliders.Length);
					_colliders2D.AddRange(all2DColliders);
				}
